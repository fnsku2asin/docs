...
<Info>
  **Multi-marketplace requests:** pass up to **4** country codes at a time in the `countries` array to scan a single barcode across several Amazon marketplaces in one request. **4 codes is the maximum per request.**
</Info>
